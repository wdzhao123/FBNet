...
class Ship():
    def __init__(self,root, is_train = True, data_len = None):
        ...
        # 获得train_file_list
        train_file_txt = open(os.path.join(self.root, 'anno', 'LFS_train.txt'))
        train_file_list = []
        train_label_list = []
        ...
        # 获得test_file_list
        test_file_txt = open(os.path.join(self.root, 'anno', 'LFS_test.txt'))
        test_file_list = []
        test_label_list = []
        ...
        # 获得图片和标签
        if self.is_train:
            self.train_img = [scipy.misc.imread(os.path.join(self.root, 'LFS_train', train_file)) for train_file in
                              train_file_list[:data_len]]
            self.train_label = train_label_list[:data_len]
        if not self.is_train:
            self.test_img = [scipy.misc.imread(os.path.join(self.root, 'LFS_test', test_file)) for test_file in
