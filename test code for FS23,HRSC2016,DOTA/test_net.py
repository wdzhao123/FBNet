...
class SI_help_res_copy(nn.Module):

    def __init__(self, block, layers,  num_classes=23, nf=50, num_modules=4, out_nc=3):#
        self.inplanes = 64
        super(SI_help_res_copy, self).__init__()
        self.RFDN1 = RFDN1()
        self.RFDN1.load_state_dict(torch.load('./超分网络模型参数'))
        ...
        self.upsampler1 = upsample_block(nf, 3, upscale_factor=4)
        self.scale_idx = 0
    ...
    def forward(self, input):
        ...
        out_lr = self.LR_conv(out_B) + B  # COMPARE1
        all = SI_map + out_lr       # max
        map = self.cos(out_lr, SI_map)
        F = map * SI_map + out_lr

        # mid4 = self.max_min(all,out_lr,SI_map, all, tag='SISR+DISR')
        # mid2 = self.max_min(out_lr,out_lr,SI_map, all, tag='DISR')
        # mid1 = self.max_min(SI_map,out_lr,SI_map, all, tag='SISR')
        # mid3 = self.max_min(F,out_lr,SI_map, all,tag='SISR_DISR')
